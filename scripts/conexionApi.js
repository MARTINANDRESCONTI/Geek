export async function listaDeProductos() {
    const conexion = await fetch("https://geek-api.vercel.app/product/");
...
const nuevoProducto = async (name, price, imagen) => {
    try {
        const res = await fetch("https://geek-api.vercel.app/product/", {
            method: "POST",
            headers: {
...
const borrarProducto = async (id) => {
    try {
        const res = await fetch(`https://geek-api.vercel.app/product/${id}`, {
            method: "DELETE",
        });
        return res.ok;
    } catch (err) {
